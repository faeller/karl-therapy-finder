  ...
  <div class="relative">
    ...
    <div v-if="pending" class="max-w-4xl mx-auto px-4 py-8">
      <div class="text-center">
        ...
        <p class="text-blue-200 text-sm">Lade Finanzierungsstatus...</p>
      </div>
    </div>

    <!-- Error State -->
    <div v-else-if="error" class="max-w-4xl mx-auto px-4 py-8">
      <div class="rounded-xl bg-white/10 backdrop-blur-sm p-6 border border-red-500/40">
    ...
    <!-- Main Content -->
    <div v-else class="max-w-4xl mx-auto px-4 pb-16 sm:px-6 lg:px-8">
      <!-- Status Overview -->
      <div class="mb-12">
          ...
          <div class="text-center mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
              <div class="bg-white/5 border border-white/10 rounded-xl p-4">
                <div class="text-2xl font-bold text-white">{{ formatEuro(fundingData.funding.received) }}</div>
                <div class="text-blue-200/70 text-sm">Erhalten</div>
              </div>
              <div class="bg-white/5 border border-white/10 rounded-xl p-4">
                <div class="text-2xl font-bold text-white">{{ formatEuro(fundingData.funding.required) }}</div>
              ...
              <div class="bg-white/5 border border-white/10 rounded-xl p-4">
                <div :class="fundingData.funding.isCovered ? 'text-green-400' : 'text-orange-400'" class="text-2xl font-bold">
                  {{ fundingData.funding.isCovered ? '0€' : formatEuro(fundingData.funding.deficit) }}
                </div>
            ...
            <div class="mt-6">
              <div class="w-full bg-blue-900/30 rounded-full h-3">
                <div 
                  ...
                  :style="{ width: `${Math.min(100, (fundingData.funding.received / fundingData.funding.required) * 100)}%` }"
                ></div>
              </div>
              <div class="text-xs text-blue-200/60 text-center mt-2">
                {{ Math.round((fundingData.funding.received / fundingData.funding.required) * 100) }}% finanziert
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Patreon Status (if available) -->
      <div v-if="fundingData.patreon" class="mb-12">
        <div class="rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 p-8">
      ...
      <!-- Cost Breakdown -->
      <div class="mb-12">
        <h3 class="text-2xl font-bold text-white mb-6 text-center">Monatliche Kosten</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div class="rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 p-6">
            ...
            <div class="text-2xl font-bold text-white mb-2">{{ formatEuro(fundingData.costs.phoneNumber) }}</div>
          ...
          <div class="rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 p-6">
            ...
            <div class="text-2xl font-bold text-white mb-2">{{ formatEuro(fundingData.costs.domain) }}</div>
          ...
          <div class="rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 p-6">
            ...
            <div class="text-2xl font-bold text-white mb-2">{{ formatEuro(fundingData.costs.calling) }}</div>
          ...
          <div class="rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 p-6">
            ...
            <div class="text-2xl font-bold text-white mb-2">{{ formatEuro(fundingData.costs.hosting) }}</div>
      ...
      <div class="text-center mb-12">
        <div class="rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 p-8">
          <h3 class="text-2xl font-bold text-white mb-4">Unterstütze KARL</h3>
          <p class="text-blue-200/80 mb-6 max-w-2xl mx-auto">
      ...
      <div class="mb-12">
        ...
        <div v-if="historyPending" class="text-center py-8">
          <UIcon name="i-heroicons-arrow-path" class="w-6 h-6 animate-spin text-blue-400 mx-auto mb-2" />
          <p class="text-blue-200/70 text-sm">Lade Verlauf...</p>
        </div>
        
        <!-- History content -->
        <div v-else-if="historyData?.history?.length > 0" class="space-y-4">
          <!-- Summary stats -->
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white/5 border border-white/10 rounded-lg p-4 text-center">
              <div class="text-lg font-bold text-white">{{ formatEuro(historyData.summary.totalReceived) }}</div>
              <div class="text-xs text-blue-200/70">Gesamt erhalten</div>
            </div>
            <div class="bg-white/5 border border-white/10 rounded-lg p-4 text-center">
              <div class="text-lg font-bold text-white">{{ formatEuro(historyData.summary.averageMonthlyReceived) }}</div>
              <div class="text-xs text-blue-200/70">⌀ pro Monat</div>
            </div>
            <div class="bg-white/5 border border-white/10 rounded-lg p-4 text-center">
              <div class="text-lg font-bold text-white">{{ historyData.summary.monthsTracked }}</div>
              <div class="text-xs text-blue-200/70">Monate erfasst</div>
            </div>
            <div class="bg-white/5 border border-white/10 rounded-lg p-4 text-center">
              <div class="text-lg font-bold text-white">{{ Math.round((historyData.summary.totalReceived / historyData.summary.totalRequired) * 100) }}%</div>
...
// Fetch funding status with SSR using Nuxt's $fetch composable
const { data: fundingData, pending, error } = await useFetch('/api/funding/status')

// Fetch funding history
const { data: historyData, pending: historyPending, error: historyError } = await useFetch('/api/funding/history')
